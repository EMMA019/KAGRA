...
"""
VRM 顔トラッキングシステム (MediaPipe Face Mesh) - 調整版
...
調整点:
    - 口の開きしきい値: 30 → 60 （閉じている時の判定を緩和）
    - まばたき感度: 3.0 → 5.0 （開いている時の判定を緩和）
...
try:
    ...
    # MediaPipe 0.10.x 用: python.solutions を直接インポート
    import mediapipe.python.solutions as mp_solutions
    mp.solutions = mp_solutions          # これで mp.solutions.face_mesh 等が使える
    HAS_MEDIAPIPE = True
except ImportError:
    HAS_MEDIAPIPE = False
...
class FaceTracker:
    ...
    _POSE_INDICES = [1, 152, 33, 263, 61, 291, 199]
    ...
    def __init__(
        ...
        if not HAS_MEDIAPIPE:
            raise ImportError("mediapipe がインストールされていません。 pip install mediapipe opencv-python")
        if not HAS_CV2:
            raise ImportError("opencv-python がインストールされていません。 pip install opencv-python")
    ...
    def _process_landmarks(self, landmarks) -> dict:
        ...
        # ── 目の開き ──────────────────────────────────────────
        eye_l_open = self._eye_openness(lm, self._LEFT_EYE_TOP, self._LEFT_EYE_BOTTOM,
                                        self._LEFT_EYE_INNER, self._LEFT_EYE_OUTER)
        eye_r_open = self._eye_openness(lm, self._RIGHT_EYE_TOP, self._RIGHT_EYE_BOTTOM,
                                        self._RIGHT_EYE_INNER, self._RIGHT_EYE_OUTER)
        # 【調整】感度を 3.0 → 5.0 に変更
        eye_l = 1.0 - min(1.0, max(0.0, eye_l_open * 5.0))
        eye_r = 1.0 - min(1.0, max(0.0, eye_r_open * 5.0))
        ...
        # 【調整】しきい値を 30 → 60 に変更
        mouth_open = min(1.0, mouth_dist / 60.0)

        left_mouth  = lm[self._LEFT_MOUTH]
        right_mouth = lm[self._RIGHT_MOUTH]
        mouth_w = abs(left_mouth.x - right_mouth.x) * w
        eye_dist = abs(lm[self._LEFT_EYE_OUTER].x - lm[self._RIGHT_EYE_OUTER].x) * w
        mouth_wide = min(1.0, mouth_w / (eye_dist * 0.8)) if eye_dist > 0 else 0.0
    ...
    def _apply_to_vrm(self, data: dict, dt: float):
        ...
        import kagra
        for bs in filter(None, [avatar._blink_l]):
            if bs:
                kagra.get_engine().set_blend_shape(vrm_id, bs, max(data["eye_l"], data["eye_r"]))
        ...
        for vowel, weight in shapes.items():
            ...
            for candidate in self._vowel_candidates(vowel):
                try:
                    kagra.get_engine().set_blend_shape(vrm_id, candidate, weight)
                except Exception:
                    pass
        ...
        if brow_avg > 0.3:
            for s in ["surprised", "Surprised", "Fcl_ALL_Surprised"]:
                try:
                    kagra.get_engine().set_blend_shape(vrm_id, s, min(1.0, brow_avg * 1.5))
                except Exception:
                    pass
        elif brow_avg < -0.2:
            for s in ["angry", "Angry", "Fcl_ALL_Angry"]:
                try:
                    kagra.get_engine().set_blend_shape(vrm_id, s, min(1.0, -brow_avg * 2.0))
                except Exception:
                    pass
        ...
        if mw > 0.6 and mo < 0.3:
            for s in ["happy", "Joy", "Fcl_ALL_Joy", "joy"]:
                try:
                    kagra.get_engine().set_blend_shape(vrm_id, s, min(1.0, mw * 1.2))
                except Exception:
                    pass
    ...
    def update(self, dt: float):
        ...
        filtered_head = self._filter_head.update(
            [raw["head_rx"], raw["head_ry"], raw["head_rz"]], dt)
        filtered_mouth = self._filter_mouth.update(
            [raw["mouth_open"], raw["mouth_wide"]], dt)
        filtered_brow = self._filter_brow.update(
            [raw["brow_l"], raw["brow_r"]], dt)

        self.head_rx, self.head_ry, self.head_rz = filtered_head
        self.mouth_open, self.mouth_wide = filtered_mouth
        self.brow_l, self.brow_r = filtered_brow
    ...
    def draw_debug(self, x: int = 0, y: int = 0, scale: float = 0.3):
        ...
        import kagra
        frame_bgr = cv2.cvtColor(self._last_frame.copy(), cv2.COLOR_RGB2BGR)
        info = [
            ...
            f"Brow: L={self.brow_l:.2f} R={self.brow_r:.2f}",
        ]
        ...
        dw = int(w * scale)
        dh = int(h * scale)
        frame_small = cv2.resize(frame_bgr, (dw, dh))
        ...
        kagra.text(info[3], x + 4, y + 60, 11, (200, 200, 200))

    def draw_debug_window(self):
        if self._last_frame is None:
            return
        frame = cv2.cvtColor(self._last_frame.copy(), cv2.COLOR_RGB2BGR)
        info = [
            f"Head: RX={self.head_rx:.2f} RY={self.head_ry:.2f} RZ={self.head_rz:.2f}",
            f"Eye: L={self.eye_blink_l:.2f} R={self.eye_blink_r:.2f}",
            f"Mouth: open={self.mouth_open:.2f} wide={self.mouth_wide:.2f}",
        ...
        for i, text in enumerate(info):
            cv2.putText(frame, text, (10, 30 + i * 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        cv2.imshow("FaceTracker Debug", frame)
        cv2.waitKey(1)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(f"  mediapipe:  {'OK' if HAS_MEDIAPIPE else 'NOT FOUND'}")
    print(f"  opencv:     {'OK' if HAS_CV2 else 'NOT FOUND'}")

    if not HAS_MEDIAPIPE or not HAS_CV2:
        print("依存ライブラリが不足しています:")
        if not HAS_MEDIAPIPE: print("  - mediapipe")
        if not HAS_CV2:       print("  - opencv-python")
        print("  pip install mediapipe opencv-python")
        exit(1)

    import sys
    camera_id = int(sys.argv[1]) if len(sys.argv) > 1 else 0
    print(f"カメラテストを開始します (カメラID={camera_id})")
