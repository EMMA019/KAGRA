...
pub fn init_rest(state: &mut SpringBoneState, world_mats: &[Matrix4<f32>]) {
    for chain in &mut state.chains {
        for i in 0..chain.joints.len().saturating_sub(1) {
            ...
            chain.joints[i + 1].bone_length = if bone_len > 0.001 { bone_len } else { 0.07 };
        ...
        for j in &mut chain.joints {
            if j.node_idx < world_mats.len() {
                let p = world_pos(&world_mats[j.node_idx]);
                j.curr = p;
                j.prev = p;
                j.target = p;
            }
...
pub fn step(
    ...
    for ci in 0..n_chains {
        ...
        for i in 0..n_joints {
            ...
            let spring = vsc(vsub(target, j.curr), j.stiffness);
            let external = vadd(vsc(j.gravity, dt * dt), vsc(wind, dt * dt * 0.5));
