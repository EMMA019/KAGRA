...
| Stage.draw fog sandwich works at GPU time | **Discarded — this was #81's miss.** `draw_mesh_3d` **queues**. Restore `set_fog(True)` runs before `render()` flush, so the sky draws with fog ON (radius 140 > fog_end 102 → 100% fog colour). Pass `skip_fog=True` on the sky command. |
| Infer skip_fog from fog-off at queue time | **Discarded after CI.** Default `fog_params.z` is 0, so every Mesh3D went unlit. Goldens: `mesh3d` mean_abs=14.5; indoor_spot/outdoor_crawl on=off. |
...
- Mesh3DCommand `skip_fog` is explicit on `sky()` / `Stage.draw` (`draw_mesh_3d(..., skip_fog=True)`). Inferring it from fog-off unlit every Mesh3D. Shader: `mesh_mat.base.w < 0.5` → unlit albedo, no fog, no ACES.
...
- CI golden failed on the first skip_fog snapshot: fog-off is the default, so floors/boxes rendered unlit. Flag is now opt-in on sky/backdrop only.
