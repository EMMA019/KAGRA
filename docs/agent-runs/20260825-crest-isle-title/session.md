...
- GPU-free source test `test_title_draw_skips_live_world` in `tests/test_open_world.py` (same style as the other Crest Isle source asserts). First CI (`python-unit` 3.11) failed: `"118" not in title_arm` matched the comment `alpha-118 fill`. Assert the `_banner` call has `overlay_alpha=255` and not `overlay_alpha=118`; do not substring-search bare digits. SMOKE verify still covers play only.
