...
- GPU smoke: this cloud rustc is 1.83; `Cargo.lock` already pins `hashbrown 0.17.1` (edition2024). `maturin develop` / `kagra.verify` could not run here. CI / Emma Windows is the GPU clock. Headless metric shipped: `kagra.vrm_gpu_stats()` + `scratch/multi_avatar_stats.json`.
