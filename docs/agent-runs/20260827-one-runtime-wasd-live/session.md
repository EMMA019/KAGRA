...
- `Renderer::new_for_window` is `cfg(not(target_arch = "wasm32"))`. Folded #103 extras: native-only `winit` dev-dep + wasm stub `main` on example `window` (WASD path stays desktop).
