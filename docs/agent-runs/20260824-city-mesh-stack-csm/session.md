...
- 薄い床メッシュは Y 幅 0 で AABB が重ならず落ちた。パッド 0.08。
- 街 JSON の y を 0.3 付近に固定すると北東の丘（高さ ≈ 3）に埋まる。y=0 は地面に載せる。
- CI: `[(focus, near_h), far]` をタプルに書いて型不一致。影マップ view をクロージャで作ると `tex` をムーブできず。
