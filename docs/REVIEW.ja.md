...
0.1.3（PyPI）+ ソース `master`（#52〜#59）。
最終目標と次の手は [ROADMAP.ja.md](ROADMAP.ja.md)。ここは能力とギャップ。

## 1 行

**最終目標は「Python で AI に体を与えるなら KAGRA」という第一想起。**
エンジン到達点は **80%**（今 **約 33%**）。100% は Python だけで three-vrm + three.js + Ursina を置き換えて困らないこと。
体は約 80% で足りる。絵は約 25% で本体。室内スポット影は CI で mean_abs=0.614（要 4.0）。
ACES と金属差は CI で出た。エージェントループは他に無い。頭脳の面は `kagra.brain("kairi")`（既定 https://kairi.onrender.com。モデルは wheel に無い）。出せるゲームが jam の箱ならループだけでは人は来ない。
...
| **three.js** | 同じ種類の仕事（ライト・影・IBL・マテリアル）。ブラウザ対決ではない | 絵は約 25%。80% には室内影の画素、這い、法線画素、局所ライト複数が要る。ACES / 金属差は CI 通過 |
| **Ursina** | 「短い Python でゲームを書く」 | API は約 60%。80% には 3 見本の 30 秒と親子 4 段。機能欠落より見本 |
...
6. **自制。** 今 33% を 80% と呼ばない。箱部屋 4 本目を D-6 と呼ばない。北極星を機能数にしない。
...
### 絵（three.js。今 25% → 80% で 85%）

| 項目 | 今 | 80% で要る | 80% の外 |
|---|---|---|---|
| ライト | 平行光 1 + 点またはスポット 1 + 半球 | 局所 4 まで。キーは影 | 多数ディレクショナル、RectArea |
| IBL | HDRI + irradiance + 4 mip。金属差は CI 通過 | 維持 | 映画のフル LOD |
| 影 | 2048×2。室内はシェーダ済み、画素は mean_abs=0.614（要 4.0） | ランプのウンブラが見える。這わない | 4 段 CSM、SSAO |
| 汎用メッシュ | baseColor + 金属/粗さ + 法線 API | `normalTexture` が画素で見える | ディスプレースメント、transmission |
| ポスト | bloom / vignette / fog / ACES（CI 通過） | 維持 | アウトライン（VRM 以外）、ボリュメトリック |
| シーングラフ | `Prop` 孫まで | 4 段（曾孫） | 無限 Object3D |
| カメラ | follow / 一人称 + ロック API | FPS として使える | 直交 3D エディタ |

絵の残りはこの順: **室内影の画素 → 這い → 法線画素 → 局所ライト複数 → 親子 4 段。**
...
次の手はロードマップの番号付きリスト。
**今約 33%。80% の本体は絵。最初は室内スポット影の画素（0.614 → 4.0）。**
