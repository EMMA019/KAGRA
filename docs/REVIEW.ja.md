...
**VRM の体は three-vrm 級に近い。3D の絵は three.js の初日を抜け始めた（トーンマップはオプトイン、GPU 未確認）。ゲームの書き味は Ursina の初日を抜け始めた（ロック・クリック・animate はある。見知らぬ人テストはまだ）。エージェント開発ループは他に無いが、出せるゲームが jam の箱なのでループだけでは人は来ない。**
