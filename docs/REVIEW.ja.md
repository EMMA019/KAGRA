...
体は約 80% で足りる。絵は約 25% で本体。室内スポットは横ランプ + 濃いウンブラに組み直し（前回 CI mean_abs=0.614 / 要 4.0）。
...
| 項目 | 今 | 80% で要る | 80% の外 |
|---|---|---|---|
| ライト | 平行光 1 + 局所 4（`slot=`。画素は CI） | 局所 4 まで。キーは影 | 多数ディレクショナル、RectArea |
| IBL | HDRI + irradiance + 4 mip。金属差は CI 通過 | 維持 | 映画のフル LOD |
| 影 | 2048×2。室内は横ランプ + 濃いウンブラに組み直し（前回 mean_abs=0.614） | ランプのウンブラが見える。這わない | 4 段 CSM、SSAO |
...
絵の残りはこの順: **室内影の画素 → 這い → 法線画素 → 局所 4 の画素。**
