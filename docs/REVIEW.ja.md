...
エンジン到達点は **80%**（今 **約 40%**）。100% は Python だけで three-vrm + three.js + Ursina を置き換えて困らないこと。
体は約 80% で足りる。絵は約 40% で本体。室内スポットと法線の画素は CI 通過（#61）。
ACES と金属差は CI で出た。這いと局所 4 の画素が次。エージェントループは他に無い。頭脳の面は `kagra.brain("kairi")`（既定 https://kairi.onrender.com。モデルは wheel に無い）。出せるゲームが jam の箱ならループだけでは人は来ない。
...
| **three.js** | 同じ種類の仕事（ライト・影・IBL・マテリアル）。ブラウザ対決ではない | 絵は約 40%。室内影と法線は CI 通過。80% には這いと局所 4 の画素。ACES / 金属差は維持 |
...
6. **自制。** 今 40% を 80% と呼ばない。箱部屋 4 本目を D-6 と呼ばない。北極星を機能数にしない。
...
次の手はロードマップの番号付きリスト。
**今約 40%。80% の本体は絵。室内影と法線は閉じた。次は這いと局所 4。**
