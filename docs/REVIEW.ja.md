...
体は three-vrm 級に近い。絵と書き味は usable-week の API まで来た（室内影 / トーンマップはペアワイズ golden。CI 待ち。法線と USB パッドの API は載った）。エージェントループは他に無い。頭脳の公式面は `kagra.brain("kairi")`（既定 https://kairi.onrender.com。モデルは wheel に無い）。出せるゲームが jam の箱ならループだけでは人は来ない。
...
| **three.js** | 同じ種類の仕事（ライト・影・IBL・マテリアル）。ブラウザ対決ではない | **API は普段の仕事に近づいた。室内影はスポットに掛かる。** トーンマップ opt-in、スペキュラ mip、2 段スナップ。法線 API は cotangent frame |
...
|---|---|---|---|
| ライト | 平行光 1 + 点またはスポット 1 + 半球。室内スポットは透視影（影はスポットに掛かる） | 室内のスポット影が画素で見える（ペアワイズ golden。CI 待ち） | 多数のディレクショナル |
| IBL | HDRI + irradiance + 4 mip スペキュラ | 金属がプラスチックに見えない（ペアワイズ `ibl_metal`。CI 待ち） | 映画のフル LOD |
...
| ポスト | bloom / vignette / fog / ACES opt-in | 真っ白・泥が止まる（ペアワイズ `tonemap_on`。CI 待ち） | アウトライン（VRM 以外） |
...
| D エージェントがゲームを | 箱部屋 3 本 + Garden / Pretty Room / Overworld（API は載った。室内影 / トーンマップはペアワイズ golden） | 見知らぬ人に見せられない | **3 見本の画素。そのあと D-6** |
...
次の手はロードマップの番号付きリスト。
**頭脳の面は載った。法線と USB の API も載った。室内影 / トーンマップはペアワイズ golden（CI）。残りは IBL・這い → 3 PyPI → 4 D-6 → 5 楔 C ワンライナー → 6 Stage 1。**
