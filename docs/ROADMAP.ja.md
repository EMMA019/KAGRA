...
- [ ] 背景: 紫単色をやめる。プロシージャルなグラデーション空 + 弱いビネット
      （テクスチャ同梱はしない。wheel ~5MB 死守）
- [ ] ライト: リムライト（逆光）でキャラを背景から剥がす。床スポットの色を
      キャラ側にも反映して「その場に立っている」感を出す
- [ ] カメラ: `cam.showcase()` プリセット — 低速オービットに顔寄り⇔全身の
      カット割りを混ぜる。デモの既定はこれ（`--no-orbit` は残す）
- [ ] リップシンク: WAV 経路に母音推定（A/I/U/E/O）。VOICEVOX mora 経路は
      あるので、デモ曲側に mora タイミングを付けて初回 DL に含める
- [ ] 足の接地: ダンス中のすべり・床めり込みの軽減（IK は既にある）
- [ ] HUD: 無地の "KAGRA" テキストをやめ、ロゴ風タイトル + 「♪ Now Playing」
      （StreamHud を非 --stream 時にも軽く流用）
