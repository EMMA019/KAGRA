...
def test_crest_isle_sun_and_ibl_are_sane():
    src = (_ROOT / "old" / "examples" / "vrm_open_world.py").read_text(encoding="utf-8")
...
def test_game_file_uses_only_public_imports():
    src = _ROOT / "old" / "examples" / "vrm_open_world.py"
...
def test_readme_sample_line():
    ...
    assert "python old/examples/vrm_open_world.py" in readme
    assert "python -m kagra.play_world" in readme
    ja = (_ROOT / "README.ja.md").read_text(encoding="utf-8")
    assert "python old/examples/vrm_open_world.py" in ja
...
def test_crest_grass_tint_is_green_not_white():
    ...
    relic = (_ROOT / "old" / "examples" / "vrm_relic_run.py").read_text(encoding="utf-8")
