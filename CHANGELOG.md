...
- One runtime (M2 first slice, schema): persistent ``WorldDoc`` in ``kagra-shared`` matches ``docs/schemas/world.json`` version 1 (stable string ids, props, parent, heightfield, lights, cameras, walkers). ``WorldDoc::from_json`` / ``to_json`` roundtrip dump JSON. ``WorldDoc::compile_scene`` builds a one-frame ``Scene3D`` (camera + batches; capsules / box / sphere primitives). ``Scene3D`` stays a draw list — dump JSON is not stuffed into it. Python dump stays the exporter; no new public game API. Renderer switch / wgpu mix / fake-headless ``(-12800,-12800)`` / VRM-on-Wasm / Crest Isle UV stream are not this slice.
